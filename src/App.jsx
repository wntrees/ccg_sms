...
function PinGate({ onUnlock }) {
  ...
  const dots = Array.from({ length: Math.max(CORRECT_PIN.length, 4) });

  return (
    <div style={{ minHeight: "100vh", background: C.g900, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: F.b }}>
      {/* Subtle background pattern */}
      <div style={{ position: "absolute", inset: 0, backgroundImage: `radial-gradient(circle at 25% 25%, rgba(201,168,76,0.04) 0%, transparent 50%), radial-gradient(circle at 75% 75%, rgba(62,122,59,0.08) 0%, transparent 50%)`, pointerEvents: "none" }} />

      <div style={{ position: "relative", textAlign: "center", width: "100%", maxWidth: 340, padding: "0 24px" }}>
        {/* Logo */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 36 }}>
          <LeafIcon size={28} color={C.gold} />
          <span style={{ fontFamily: F.d, fontSize: 24, color: C.white, fontWeight: 600 }}>Canopy Capital</span>
        </div>

        {/* Card */}
        <div style={{ background: "rgba(255,255,255,0.04)", border: `1px solid rgba(201,168,76,0.2)`, borderRadius: 20, padding: "32px 28px", backdropFilter: "blur(8px)" }}>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", marginBottom: 28, letterSpacing: "0.04em", textTransform: "uppercase" }}>Enter your PIN to continue</p>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 14, marginBottom: 28 }}>
            {dots.map((_, i) => (
              <div key={i} style={{
                ...
                background: i < pin.length ? (error ? "#e24b4a" : C.gold) : "rgba(255,255,255,0.15)",
                border: `1.5px solid ${i < pin.length ? (error ? "#e24b4a" : C.gold) : "rgba(255,255,255,0.2)"}`,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
            {["1","2","3","4","5","6","7","8","9","","0","⌫"].map((k, i) => (
              <button
                key={i}
                onClick={() => k === "⌫" ? handleBackspace() : k ? handleKey(k) : null}
                disabled={!k}
                style={{
                  height: 52, borderRadius: 10,
                  background: k === "⌫" ? "rgba(201,168,76,0.1)" : k ? "rgba(255,255,255,0.06)" : "transparent",
                  border: k ? `1px solid ${k === "⌫" ? "rgba(201,168,76,0.25)" : "rgba(255,255,255,0.1)"}` : "none",
                  color: k === "⌫" ? C.gold : C.white,
                  fontSize: k === "⌫" ? 18 : 20,
                  fontFamily: F.b,
                  fontWeight: 400,
                ...
                onMouseEnter={(e) => { if (k) e.currentTarget.style.background = k === "⌫" ? "rgba(201,168,76,0.18)" : "rgba(255,255,255,0.12)"; }}
                onMouseLeave={(e) => { if (k) e.currentTarget.style.background = k === "⌫" ? "rgba(201,168,76,0.1)" : "rgba(255,255,255,0.06)"; }}
              >
                {k}
              </button>
            ))}
          ...
          {error && (
            <p style={{ color: "#e24b4a", fontSize: 12, marginTop: 16, textAlign: "center" }}>Incorrect PIN — try again</p>
          )}
        </div>

        <p style={{ fontFamily: F.d, fontSize: 12, color: "rgba(255,255,255,0.2)", marginTop: 28, fontStyle: "italic" }}>
